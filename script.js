// Sample project JavaScript. Replace `sampleData` with your chat JSON.
...
function buildDial(moments){
  ...
  moments.forEach((m,i)=>{
    ...
    dot.setAttribute('fill','url(#grad'+i+')');
    dot.setAttribute('class','dial-dot');
    ...
    lab.setAttribute('x', cx + Math.cos(angle)*(radius+28));
    lab.setAttribute('y', cy + Math.sin(angle)*(radius+28));
    lab.setAttribute('text-anchor','middle');
    lab.setAttribute('alignment-baseline','middle');
    lab.setAttribute('font-size','11');
    lab.setAttribute('fill','rgba(255,255,255,0.8)');
    lab.textContent = m.label;
    timeDial.appendChild(lab);
  });
}
...
// Show bubble with messages (typewriter)
function showBubble(moment){
  ...
  function nextMsg(){
    ...
    typeWriter(msg.text, div, ()=>{
      i++; chatBody.scrollTop = chatBody.scrollHeight; setTimeout(nextMsg, 400);
    });
  }
  nextMsg();
}
...
// Simple typewriter into element
function typeWriter(text, el, cb){
  ...
  const t = setInterval(()=>{
    ...
    j++;
    if(j>=text.length){ clearInterval(t); if(cb) cb(); }
  }, speed);
}

// Play Journey — auto-play moments sequentially
